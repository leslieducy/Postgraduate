...
x_train,val_x,x_test=np.reshape(train_set[0],(-1,3,128,128)).swapaxes(1,3),np.reshape(val_set[0],(-1,3,128,128)).swapaxes(1,3),np.reshape(test_set[0],(-1,3,128,128)).swapaxes(1,3)
...
model = Sequential()
model.add(Conv2D(filters = 3, kernel_size = (3,3),data_format='channels_first', padding = 'same', activation = 'relu',input_shape = (128, 128, 3)))
