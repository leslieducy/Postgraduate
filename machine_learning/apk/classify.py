...
list_train = []
list_res_train = []
with open("prunedata/train.csv","r") as csvfile:
...
csvfile.close()
with open("prunedata/trainresult.csv","r") as csvfile:
    reader = csv.reader(csvfile)
    #这里不需要readlines
    for line in reader:
        line_int = list(map(lambda x:int(float(x)),line))
...
list_test = []
list_res_test = []
with open("prunedata/test.csv","r") as csvfile:
    reader = csv.reader(csvfile)
    #这里不需要readlines
    for line in reader:
        line_int = list(map(lambda x:int(float(x)),line))
...
csvfile.close()
with open("prunedata/testresult.csv","r") as csvfile:
    reader = csv.reader(csvfile)
    #这里不需要readlines
    for line in reader:
        line_int = list(map(lambda x:int(float(x)),line))
...
# # 写入判别数组
# csv_pro_file = open("proba.csv", "w", newline='')
